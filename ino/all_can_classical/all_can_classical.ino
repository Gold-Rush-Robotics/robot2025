...
void loop () {
  ...
  data[2]=6;
  data[3]=9;
  memcpy(message_send.data16,data,sizeof(data));
  
//  message.rtr = true ;
//--- CAN3 send
  if (gSentCount3 < SEND_COUNT) {
    message_send.id = 0 ; //(micros () % 682) * 3 + 0 ;
    const bool ok = ACAN_T4::can3.tryToSend (message_send) ;
    if (ok) {
      gSentCount3 += 1 ;
    }
  }
//--- CAN1 send
  if (gSentCount1 < SEND_COUNT) {
    message_send.id = 1 ; // (micros () % 682) * 3 + 1 ;
    const bool ok = ACAN_T4::can1.tryToSend (message_send) ;
    if (ok) {
      gSentCount1 += 1 ;
    }
  }
